 ...
 * 
 * POST /api/emails/send - Send an immediate email
 * 
...
import { sendEmailSchema } from '@/lib/validators/email.schemas';

/**
 * POST /api/emails/send
 * 
 * Send an immediate email (transactional or marketing)
 * 
 * Request body:
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (validatedData.type === 'transactional') {
      ...
      result = await emailService.sendMarketingEmail({
        to: validatedData.to,
        subject,
        html,
        text,
