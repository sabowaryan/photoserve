...
import { ArrowLeft, ChevronRight, ExternalLink, Clock, ImageIcon, Eye, Calendar, BarChart3, Lock } from "lucide-react";
...
export function GalleryHero({
  ...
  const hasAnalyticsAccess = userPlan === "pro";
              ...
              <button 
                onClick={() => {
                  if (hasAnalyticsAccess) {
                    router.push(`/dashboard/gallery/${galleryId}/analytics`);
                  } else {
                    setShowUpgradeModal(true);
                  }
                }}
                className={`group relative px-4 py-2.5 backdrop-blur-md font-bold text-sm rounded-xl transition-all border hover:-translate-y-0.5 active:scale-95 flex items-center gap-2 ${
                  hasAnalyticsAccess 
                    ? 'bg-white/10 text-white border-white/20 hover:bg-white/20' 
                    : 'bg-white/5 text-white/60 border-white/10 hover:bg-white/10'
                }`}
                title={hasAnalyticsAccess ? "Voir les analytics" : "Fonctionnalité Pro"}
              >
                {hasAnalyticsAccess ? (
                  <BarChart3 size={16} />
                ) : (
                ...
                {!hasAnalyticsAccess && (
                  <span className="text-[9px] font-black bg-amber-500/20 text-amber-300 px-1.5 py-0.5 rounded-full border border-amber-400/30">
                    PRO
                  </span>
                )}
              </button>
